...
export function createHandler(deps: HandlerDependencies): CloudHandler {
  return async (event: unknown, _context: unknown): Promise<HandlerResponse> => {
    ...
    if (hasHttpShape(event)) return handleHttp(event, deps, correlationId);
...
async function handleHttp(
  ...
  if (!secretMatches(suppliedSecret, deps.webhookSecret)) return response(403, 'forbidden');

  if (bodyByteLength(event.body) > MAX_BODY_BYTES) return response(413, 'payload too large');
  let update: unknown;
  try {
...
let productionDependenciesPromise: Promise<HandlerDependencies> | undefined;

async function productionDependencies(): Promise<HandlerDependencies> {
  productionDependenciesPromise ??= initializeProductionDependencies().catch((error: unknown) => {
...
async function initializeProductionDependencies(): Promise<HandlerDependencies> {
  ...
  return { webhookSecret: config.webhookSecret, router, scheduler, outbox, newCorrelationId: newId };
}
...
export const handler: CloudHandler = async (event: unknown, context: unknown): Promise<HandlerResponse> => {
  ...
  try {
    deps = await productionDependencies();
  } catch (error) {
  ...
  return createHandler({ ...deps, newCorrelationId: () => correlationId })(event, context);
};
